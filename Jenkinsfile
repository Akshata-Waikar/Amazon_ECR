pipeline {
    ...
    environment {
        ...
        IMAGE_TAG = 'latest'
        SSH_PRIVATE_KEY = credentials('live-server-key')  // Jenkins SSH credential ID
        LIVE_SERVER_IP = '3.38.100.12'  // ✅ Replace with your actual EC2 IP
    }
        ...
        stage('Deploy to Live Server') {
            steps {
                ...
                ssh -o StrictHostKeyChecking=no -i $SSH_PRIVATE_KEY ec2-user@$LIVE_SERVER_IP << EOF
                    docker login -u AWS -p $(aws ecr get-login-password --region ap-northeast-2) $ECR_REPO
